  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-2">
      <div className="flex flex-col max-h-full items-center justify-center">
        <h1 className="text-3xl font-medium text-primary-400">No Favorites.</h1>
              ...
              <tr key={`meeting-${meeting.id}-${index}`}>
                ...
                >
                  ...
                  <RemoveButton
                    text="Fav"
                    onClick={() => {
                      handleRemoveFromFavorite(meeting).then((data) => {
                        // remove from d.b
...
export default function FavoriteCompact({
  setOverlayComponent,
  favorite,
  setFavorite,
  removeMeetingFromFavoritesById,
}) {
    ...
    setTimeout(() => {
      getMeetingFavorite()
        .then((data) => {
          ...
          setFavorite(
            data?.meeting_favorite?.map((meeting) => ({ ...meeting }))
          );
          setLoading(false);
        })
        .catch((e) => {
